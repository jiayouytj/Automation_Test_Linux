...
#date            :20190501                                                                                                #
#version         :1.0                                                                                                     #
#usage           :./all_install.sh                                                                                        #
#actual results  :Testcase all_install.sh passed!                                                                         #
#expected results:Testcase all_install.sh passed!                                                                         #
###########################################################################################################################
...
DB_rpm_LogPath="$UnifiedLogPath/installationlog/mongodb"                                                                  #the default MongoDB rpm LogPath
...
LA_rpm_LogPath="$UnifiedLogPath/installationlog/licenseagent"                                                             #the default License Agent rpm LogPath
...
ALL_LogPath="$UnifiedLogPath/installationlog/all/"                                                                        #All-in-two-linux LogPath
...
DB_systemd="/usr/lib/systemd/system/mongod.service"                                                                       #MongoDB systemd file
ES_systemd="/usr/lib/systemd/system/elasticsearch.service"                                                                #Elasticsearch systemd file
LA_systemd="/usr/lib/systemd/system/netbrainlicense.service"                                                              #License Agent systemd file
MQ_systemd="/usr/lib/systemd/system/rabbitmq-server.service"                                                              #RabbitMQ systemd file
RE_systemd="/usr/lib/systemd/system/redis.service"                                                                        #Redis systemd file
SM_systemd="/usr/lib/systemd/system/netbrainagent.service"                                                                #Service Monitor Agent systemd file
...
if ! grep -q "CPUQuota" "$DB_systemd";then                                                                                #if the testcase fails, generate the reasons for the failure
echo -e "There is no CPU limit in MongoDB systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "MemoryLimit" "$DB_systemd";then                                                                             #if the testcase fails, generate the reasons for the failure
echo -e "There is no Memory limit in MongoDB systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "CPUQuota" "$ES_systemd";then                                                                                #if the testcase fails, generate the reasons for the failure
echo -e "There is no CPU limit in Elasticsearch systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "Restart=on-failure" "$DB_systemd";then                                                                      #if the testcase fails, generate the reasons for the failure
echo -e "The restarting rule is not on-failure defined in MongoDB systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "Restart=on-failure" "$ES_systemd";then                                                                      #if the testcase fails, generate the reasons for the failure
echo -e "The restarting rule is not on-failure defined in Elasticsearch systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "Restart=on-failure" "$LA_systemd";then                                                                      #if the testcase fails, generate the reasons for the failure
echo -e "The restarting rule is not on-failure defined in License Agent systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "Restart=on-failure" "$MQ_systemd";then                                                                      #if the testcase fails, generate the reasons for the failure
echo -e "The restarting rule is not on-failure defined in RabbitMQ systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "Restart=on-failure" "$RE_systemd";then                                                                      #if the testcase fails, generate the reasons for the failure
echo -e "The restarting rule is not on-failure defined in Redis systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! grep -q "Restart=on-failure" "$SM_systemd";then                                                                      #if the testcase fails, generate the reasons for the failure
echo -e "The restarting rule is not on-failure defined in Service Monitor Agent systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if [ ! `ls -l "$UnifiedLogPath/installationlog"| grep -v '^total'|wc -l` == 3 ];then                                      #if the testcase fails, generate the reasons for the failure
echo -e "More than three files/directories under "$UnifiedLogPath/installationlog" were created. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if [ ! `ls -l "$ALL_LogPath"| grep -v '^total'|wc -l` == 1 ];then                                                         #if the testcase fails, generate the reasons for the failure
echo -e "More than one files/directories under "$ALL_LogPath" were created. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if [ ! `ls -l "$DB_rpm_LogPath"| grep -v '^total'|wc -l` == 1 ];then                                                      #if the testcase fails, generate the reasons for the failure
echo -e "More than one files/directories under "$DB_rpm_LogPath" were created. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if [ ! `ls -l "$LA_rpm_LogPath"| grep -v '^total'|wc -l` == 1 ];then                                                      #if the testcase fails, generate the reasons for the failure
echo -e "More than one files/directories under "$LA_rpm_LogPath" were created. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if [[ ! `ls -l "$ALL_LogPath"| grep "install.log"` ]];then                                                                #if the testcase fails, generate the reasons for the failure
echo -e "The install.log was not created under "$ALL_LogPath". This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if [[ ! `ls -l "$DB_rpm_LogPath"| grep "install_rpm.log"` ]];then                                                         #if the testcase fails, generate the reasons for the failure
echo -e "The install_rpm.log was not created under "$DB_rpm_LogPath". This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if [[ ! `ls -l "$LA_rpm_LogPath"| grep "install_rpm.log"` ]];then                                                         #if the testcase fails, generate the reasons for the failure
echo -e "The install_rpm.log was not created under "$LA_rpm_LogPath". This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi
...
&& grep -q "CPUQuota" "$DB_systemd" && grep -q "MemoryLimit" "$DB_systemd" && grep -q "CPUQuota" "$ES_systemd" \
&& grep -q "Restart=on-failure" "$DB_systemd" && grep -q "Restart=on-failure" "$ES_systemd" \
&& grep -q "Restart=on-failure" "$LA_systemd" && grep -q "Restart=on-failure" "$MQ_systemd" \
&& grep -q "Restart=on-failure" "$RE_systemd" && grep -q "Restart=on-failure" "$SM_systemd" \
...
&& [ `ls -l "$UnifiedLogPath/installationlog"| grep -v '^total'|wc -l` == 3 ] \
&& [ `ls -l "$ALL_LogPath"| grep -v '^total'|wc -l` == 1 ] \
&& [ `ls -l "$DB_rpm_LogPath"| grep -v '^total'|wc -l` == 1 ] \
&& [ `ls -l "$LA_rpm_LogPath"| grep -v '^total'|wc -l` == 1 ] && [[ `ls -l "$ALL_LogPath"| grep "install.log"` ]] \
&& [[ `ls -l "$DB_rpm_LogPath"| grep "install_rpm.log"` ]] && [[ `ls -l "$LA_rpm_LogPath"| grep "install_rpm.log"` ]] \
