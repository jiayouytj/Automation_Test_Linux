...
#                 while specifying bad DataPath and LogPath, the expected errors are given,                               #
#                 which means that install.sh is correctly invoked and executed.                                          #
#author          :Zihao Yan                                                                                                #
#date            :20190501                                                                                                #
#version         :1.0                                                                                                     #
#usage           :./all_install_bad_path.sh                                                                               #
#actual results  :Testcase all_install_bad_path.sh passed!                                                                #
#expected results:Testcase all_install_bad_path.sh passed!                                                                #
###########################################################################################################################
...
echo -e "/" >>$input                                                                                                      #use a bad Data Path for NetBrain
echo -e "/etc" >>$input                                                                                                   #use a bad Data Path for NetBrain
echo -e "/derek" >>$input                                                                                                 #use a bad Data Path for NetBrain
echo -e "derek" >>$input                                                                                                  #use a bad Data Path for NetBrain
echo -e "" >>$input                                                                                                       #use the default Data Path for NetBrain
echo -e "/" >>$input                                                                                                      #use a bad Log Path for NetBrain
echo -e "/etc" >>$input                                                                                                   #use a bad Log Path for NetBrain
echo -e "/derek" >>$input                                                                                                 #use a bad Log Path for NetBrain
echo -e "derek" >>$input                                                                                                  #use a bad Log Path for NetBrain
...
msg_error1="The path (/) is invalid, it should be at least 2 level directory"                                             #message for a sign of invalid path
msg_error2="The path (/etc) is invalid, it should be at least 2 level directory"                                          #message for a sign of invalid path
msg_error3="The path (/derek) is invalid, it should be at least 2 level directory"                                        #message for a sign of invalid path
msg_error4="The path parameter is not an absolute path"                                                                   #message for a sign of invalid path
...
timeout 1200 `pwd`/install.sh <$input >$log 2>&1                                                                          #execute netbrain-all-in-two-linux installation script and save the log
...
echo -e "The number of bad path output 1 are not 2. This may indicate \
that the all-in-two-linux installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! [[ `grep "$msg_error2" "$log"|wc -l` == 2 ]];then                                                                    #if the testcase fails, generate the reasons for the failure
echo -e "The number of bad path output 2 are not 2. This may indicate \
that the all-in-two-linux installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! [[ `grep "$msg_error3" "$log"|wc -l` == 2 ]];then                                                                    #if the testcase fails, generate the reasons for the failure
echo -e "The number of bad path output 3 are not 2. This may indicate \
that the all-in-two-linux installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi

if ! [[ `grep "$msg_error4" "$log"|wc -l` == 2 ]];then                                                                    #if the testcase fails, generate the reasons for the failure
echo -e "The number of bad path output 4 are not 2. This may indicate \
that the all-in-two-linux installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi
...
&& [[ `grep "$msg_error1" "$log"|wc -l` == 2 ]] && [[ `grep "$msg_error2" "$log"|wc -l` == 2 ]] \
&& [[ `grep "$msg_error3" "$log"|wc -l` == 2 ]] && [[ `grep "$msg_error4" "$log"|wc -l` == 2 ]] \
