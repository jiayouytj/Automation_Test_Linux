...
##################################################################################################################
# Usage: ./sanity_testrun_VM.sh. This shell script will automatically copy installation packages, certificates,  #
#                           and pull all testcases and automation framework from GitHub,  then run sanity        #
...
cd $AA_testcase_dir;ls aa_install.sh aa_install_new_user_not_allowed.sh aa_install_ssl_firewall_umask_sudo.sh \
aa_uninstall.sh >../aa.list                                                                                      #generate sanity Ansible Agent testlist
cd $DB_testcase_dir;ls db_new_user_not_allowed.sh >../db.list                                                    #generate sanity MongoDB testlist
cd $ES_testcase_dir;ls es_install_new_user_not_allowed.sh >../es.list                                            #generate sanity Elasticsearch testlist
cd $LA_testcase_dir;ls la_install_new_user_not_allowed.sh >../la.list                                            #generate sanity License Agent testlist
cd $RE_testcase_dir;ls re_install_new_user_not_allowed.sh >../re.list                                            #generate sanity Redis testlist
cd $MQ_testcase_dir;ls mq_install_new_user_not_allowed.sh mq_install_ssl_firewall_umask_sudo.sh \
mq_install_clean.sh >../mq.list                                                                                  #generate sanity RabbitMQ testlist
cd $SM_testcase_dir;ls sm_install_new_user_not_allowed.sh >../sm.list                                            #generate sanity Service Monitor Agent testlist
cd $FS_testcase_dir;ls fs_install_new_user_not_allowed.sh >../fs.list                                            #generate sanity Front Server testlist
cd $ALL_testcase_dir;ls all_install_all_customized_inputs.sh all_install_ssl_firewall_umask_sudo.sh >../all.list                                                            #generate sanity all-in-two-linux testlist
...
echo "This is the sanity testrun summary on a CentOS 7.5 Minimal Version." >>$PWD/$results/$summary
fi
if hostname -I|grep "192.168.30.187" >/dev/null 2>&1;then                                                        #CentOS 7.5 Development Version is used for the testrun summary
echo "This is the sanity testrun summary on a CentOS 7.5 Development Version." >>$PWD/$results/$summary
fi
if hostname -I|grep "192.168.30.188" >/dev/null 2>&1;then                                                        #CentOS 7.6 Minimal Version is used for the testrun summary 
echo "This is the sanity testrun summary on a CentOS 7.6 Minimal Version." >>$PWD/$results/$summary
fi
if hostname -I|grep "192.168.30.189" >/dev/null 2>&1;then                                                        #CentOS 7.6 Development Version is used for the testrun summary
echo "This is the sanity testrun summary on a CentOS 7.6 Development Version." >>$PWD/$results/$summary
fi
