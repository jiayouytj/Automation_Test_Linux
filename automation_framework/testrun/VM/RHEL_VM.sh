...
##################################################################################################################
#title           :RHEL_VM.sh                                                                                     #
#description     :This script will kick of testruns of all NetBrain Linux Component under RHEL VM                #
#author		     :Zihao Yan                                                                                       #
#date            :20190306                                                                                       #
#version         :1.0                                                                                            #
#usage		     :./RHEL_VM.sh                                                                                   #
...
##################################################################################################################
# Usage: ./RHEL_VM.sh.   This shell script will automatically run a bunch of testcases located within the        #
#                        testcase directory. The summary of testrun results will be generated                    #
#                        in the same directory as this shell script located. In order to easily investigating    #
#                        any failures, the succeeded logs and failed logs are both generated.                    #
#                                                                                                                #
#                                                                                                                #
#                                                                                                                #
#                                                                                                                #
...
##################################################################################################################
# These commands are configurations for different components, such as MongoDB, and Elasticsearch                 #
##################################################################################################################
...
aa)
  ...
  ;;
all)
  DIR_name=`gzip -dc $ALL_path| tar tvf -|head -1|awk '{print $6}'|sed 's/.$//'`                                 #the unzipped installation package name for ALL-LINUX
  package="$installation_DIR/$DIR_name"                                                                          #the installation package directory after unzipping the tar.gz file
...
ARR=(es la db sm aa mq re te fs all)                                                                             #list all possible prefix for all testcases
for prefix in ${ARR[*]}
do
case $prefix in es)
