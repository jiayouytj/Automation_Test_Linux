...
##################################################################################################################
#title           :CentOS7.5_docker.sh                                                                            #
#description     :This script will kick of testruns of all NetBrain Linux Component under CentOS 7.5 docker      #
#author		     :Zihao Yan                                                                                       #
#date            :20190306                                                                                       #
#version         :1.0                                                                                            #
#usage		     :./CentOS7.5_docker.sh                                                                          #
...
##################################################################################################################
# Usage: ./CentOS7.5_docker.sh. This shell script will automatically run a bunch of testcases located within the #
#                        testcase directory. The summary of testrun results will be generated                    #
#                        in the same directory as this shell script located. The Dockerfiles are also generated  # 
#                        on the fly, and will be deleted after the testrun. In order to easily investigating     #
#                        any failures, The docker containers are stopped, but not removed after the testrun.     #
#                                                                                                                #
#                                                                                                                #
#                                                                                                                #
#                                                                                                                #
...
run_Docker()
{
  ...
  testcase=`find $testcase_DIR/docker/CentOS -name $(basename ${array[i]})`                                      #get testcase name
  ...
  docker exec -i $name bash -c "find $package -name 'uninstall.sh'|xargs sed -i '/remove_port$/d'"               #do not execute remove_all_port function in docker
...
db)
  Dockerfile="DB_"$OS_Version"_Dockerfile"                                                                       #specify the Dockerfile name for MongoDB
  DIR_name=`gzip -dc $DB_path| tar tvf -|head -1|awk '{print $6}'|sed 's/.$//'`                                  #the unzipped installation package name for MongoDB Agent
  package="$installation_DIR/$DIR_name"                                                                          #the installation package directory after unzipping the tar.gz file
