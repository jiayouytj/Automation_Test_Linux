...
#                 the License Agent Installation Path,                                                           #
#                 which means that uninstall.sh is correctly invoked and executed                                #
#author		     :Zihao Yan                                                                                       #
#date            :20190329                                                                                       #
#version         :1.0                                                                                            #
#usage		     :./la_uninstall1.sh                                                                             #
#actual results  :Testcase la_uninstall1.sh passed!                                                              #
#expected results:Testcase la_uninstall1.sh passed!                                                              #
##################################################################################################################
...
New_LogPath="/var/log/derekli/licenseagent"                                                                      #the new LogPath
sed -i "s~$LogPath~$New_LogPath~g" $conf_path                                                                    #modify the Elasticsearch conf file, use the new LogPath
...
if [ ! -d $New_LogPath ];then                                                                                    #if the testcase fails, generate the reasons for the failure
echo -e "The Log Path $New_LogPath was not created. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi
...
&& grep -q "$msg_install" $log && [ -d $New_LogPath ];then                                                       #determine if License Agent installation is successfully completed
timeout 600 echo "yes"|$UninstallPath/uninstall.sh >$log2 2>&1                                                   #execute License Agent uninstallation script and save the log    
if [ $? -eq 124 ];then                                                                                           #determine if the testcase runs timed out
    echo "Testcase $BASH_SOURCE timeout!"
    ...
    if [ -d $New_LogPath ];then                                                                                  #if the testcase fails, generate the reasons for the failure
    echo -e "The Log Path $New_LogPath still existed after uninstallation, which is not an expected behavior. \
	...
	if grep -q "$msg_uninstall" $log2 && [ ! -d $New_LogPath ] && [ ! -f $service ];then                         #determine if License Agent uninstallation is successfully completed
