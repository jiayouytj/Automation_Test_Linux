...
#                 while all input parameters are customized,                                                              #
...
UnifiedLogPath="/var/log/derek"                                                                                           #unified LogPath
UnifiedDataPath="/var/lib/derek"                                                                                          #unified DataPath
...
DEFAULT_CERT="/etc/ssl/cert.pem"                                                                                          #default Certificate Path
DEFAULT_KEY="/etc/ssl/key.pem"                                                                                            #default Private Key Path
DEFAULT_CA="/etc/ssl/ca.pem"                                                                                              #default CA Path
CERT="/opt/certificate.pem"                                                                                               #Certificate Path
KEY="/var/key.crt"                                                                                                        #Private Key Path
CA="/usr/ca-root.pem"                                                                                                     #CA Path
cleanup()
{
...
"$UnifiedSSLPath"/netbrainlicense "$UnifiedSSLPath"/netbrain "$CERT" "$KEY" "$CA" /etc/*.js                               #remove all netbrain-all-in-two-linux files and paths
...
ifconfig eth0:1 192.168.1.109 netmask 255.255.255.0                                                                       #create a virtual IP and netmask
ifconfig eth0:1 up                                                                                                        #enable the virtual IP
cp "$DEFAULT_CERT" "$CERT"                                                                                                #copy the Certificate to a new location
cp "$DEFAULT_KEY" "$KEY"                                                                                                  #copy the Private Key to a new location
cp "$DEFAULT_CA" "$CA"                                                                                                    #copy the CA to a new location
input="input.file"                                                                                                        #input file for install.sh
UserName="derek"                                                                                                          #use the customized UserName
Password="ABCDEFG1234567"                                                                                                 #NetBrain service password
...
DB_Port="10000"                                                                                                           #customized MongoDB Port
LA_Port="20000"                                                                                                           #customized License Agent Port
ES_Port="30000"                                                                                                           #customized Elasticsearch Port
MQ_Port="40000"                                                                                                           #customized RabbitMQ Port
RE_Port="50000"                                                                                                           #customized Redis Port
IP=`hostname -I|awk '{print $2}'`                                                                                         #get the second IP address of the machine
...
echo -e "$UnifiedDataPath" >>$input                                                                                       #use the customized Data Path for NetBrain
echo -e "$UnifiedLogPath" >>$input                                                                                        #use the customized Log Path for NetBrain
echo -e "$IP" >>$input                                                                                                    #use the second IP address of this machine
echo -e "$UserName" >>$input                                                                                              #use the default NetBrain service username
echo -e "$Password" >>$input                                                                                              #set the NetBrain service password
echo -e "$Password" >>$input                                                                                              #repeat the NetBrain service password
echo -e "YES" >>$input                                                                                                    #enable SSL on NetBrain Database Server
echo -e "$CERT" >>$input                                                                                                  #use the customized Certificate
echo -e "$KEY" >>$input                                                                                                   #use the customized Private Key 
echo -e "$CA" >>$input                                                                                                    #use the customized CA
echo -e "YES" >>$input                                                                                                    #use the customized server ports
echo -e "$DB_Port" >>$input                                                                                               #use the customized MongoDB ports
echo -e "$LA_Port" >>$input                                                                                               #use the customized License Agent ports
echo -e "$ES_Port" >>$input                                                                                               #use the customized Elasticsearch ports
echo -e "$MQ_Port" >>$input                                                                                               #use the customized RabbitMQ ports
echo -e "$RE_Port" >>$input                                                                                               #use the customized Redis ports
echo -e "http://172.17.0.3/" >>$input                                                                                     #URL of NetBrain Web API service
echo -e "yes" >>$input                                                                                                    #make sure that all the previous parameters are in effect
...
timeout 1200 bash -c ". install.sh <$input" >$log 2>&1                                                                    #execute netbrain-all-in-two-linux installation script and save the log
...
xargs grep -s "$Password" --binary-files=without-match` ]];then                                                           #if the testcase fails, generate the reasons for the failure
echo -e "The password $Password was not encrypted in some of the files generated. This may indicate \
...
xargs grep -s "$Password" --binary-files=without-match` ]] \
