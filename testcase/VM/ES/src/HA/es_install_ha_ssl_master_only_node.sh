...
##################################################################################################################
#title           :es_install_ha_ssl_master_only_node.sh                                                          #
...
#                 is enabled. The installation is on the master only node for Elasticsearch cluster,             #
#                 which means that install.sh is correctly invoked and executed.                                 #
#author		     :Derek Li                                                                                       #
#date            :20190613                                                                                       #
#version         :1.0                                                                                            #
#usage		     :./es_install_ha_ssl_master_only_node.sh                                                        #
#actual results  :Testcase es_install_ha_ssl_master_only_node.sh passed!                                         #
#expected results:Testcase es_install_ha_ssl_master_only_node.sh passed!                                         #
##################################################################################################################
...
sed -i "${UseSSL_line_number}s/$UseSSL/$new_UseSSL/" $conf_path                                                  #modify the Elasticsearch conf file, enable SSL
CERT="/etc/ssl/cert.pem"                                                                                         #certificate path
KEY="/etc/ssl/key.pem"                                                                                           #certificate key path
CA="/etc/ssl/ca.pem"                                                                                             #CertAuth path
...
sed -i "${Certificate_line_number}s~$Certificate~$CERT~" $conf_path                                              #modify the Elasticsearch conf file, change default Certificate
...
sed -i "${PrivateKey_line_number}s~$PrivateKey~$KEY~" $conf_path                                                 #modify the Elasticsearch conf file, change default PrivateKey
CertAuth="`grep "^CertAuth" $conf_path|tr "=" " "|awk '{print $2}'`"                                             #the default CertAuth
CertAuth_line_number=`grep -n "^CertAuth" $conf_path|cut -f 1 -d":"`                                             #get CertAuth config line number
sed -i "${CertAuth_line_number}s~$CertAuth~$CA~" $conf_path                                                      #modify the Elasticsearch conf file, change default CertAuth
...
MasterOnlyNode="`grep "^MasterOnlyNode" $conf_path|tr "=" " "|awk '{print $2}'`"                                 #the default MasterOnlyNode
MasterOnlyNode_line_number=`grep -n "^MasterOnlyNode" $conf_path|cut -f 1 -d":"`                                 #get MasterOnlyNode config line number
New_MasterOnlyNode="yes"                                                                                         #the new MasterOnlyNode which is yes
sed -i "${MasterOnlyNode_line_number}s/$MasterOnlyNode/$New_MasterOnlyNode/" $conf_path                          #modify the Elasticsearch conf file, change default MasterOnlyNode
