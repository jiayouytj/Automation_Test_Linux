...
##################################################################################################################
#title           :es_install_logpath_immutable.sh                                                                #
...
#date            :20190215                                                                                       #
#version         :1.0                                                                                            #
#usage		     :./es_install_logpath_immutable.sh                                                              #
#actual results  :Testcase es_install_logpath_immutable.sh passed!                                               #
#expected results:Testcase es_install_logpath_immutable.sh passed!                                               #
##################################################################################################################
...
rm -rf $LogPath $UnifiedConfigPath $UnifiedLogPath $UnifiedUninstallPath                                         #remove all Elasticsearch files and paths
...
LogPath="`grep "^LogPath" $conf_path|tr "=" " "|awk '{print $2}'`"                                               #default LogPath
...
if [ $? -eq 124 ];then                                                                                           #determine if the testcase runs timed out
    ...
    chattr -i $LogPath                                                                                           #make LogPath not immutable
	cleanup                                                                                                      #invoke cleanup funtion to purge all Elasticsearch related stuffs
    exit 0
fi
chattr -i $LogPath                                                                                               #make LogPath not immutable
