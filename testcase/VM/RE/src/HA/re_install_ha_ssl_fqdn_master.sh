...
##################################################################################################################
#title           :re_install_ha_ssl_fqdn_master.sh                                                               #
...
#                 is enabled. The installation is on master for Redis cluster using FQDN on MasterNode.          #
#                 which means that install.sh is correctly invoked and executed.                                 #
#author		     :Zihao Yan                                                                                       #
#date            :20190613                                                                                       #
#version         :1.0                                                                                            #
#usage		     :./re_install_ha_ssl_fqdn_master.sh                                                             #
#actual results  :Testcase re_install_ha_ssl_fqdn_master.sh passed!                                              #
#expected results:Testcase re_install_ha_ssl_fqdn_master.sh passed!                                              #
##################################################################################################################
...
RE_master_systemd="/usr/lib/systemd/system/redis-master.service"                                                 #Redis master systemd file
...
if [[ ! `ls -ld $RE_master_systemd|awk '{print $1}'|sed "s/^-//g"|sed "s/\.//g"` == $permission_644 ]];then      #if the testcase fails, generate the reasons for the failure
echo -e "The permission of $RE_master_systemd was not 644. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi
...
if ! grep -q "Restart=on-failure" "$RE_master_systemd";then                                                      #if the testcase fails, generate the reasons for the failure
echo -e "The restarting rule is not on-failure defined in redis-master systemd. This may indicate \
that the installation was not successful. This is the reason why the testcase failed.">>$log_fail
fi
...
&& [[ `ls -ld $RE_master_systemd|awk '{print $1}'|sed "s/^-//g"|sed "s/\.//g"` == $permission_644 ]] \
&& [[ `ls -ld $RE_ssl_systemd|awk '{print $1}'|sed "s/^-//g"|sed "s/\.//g"` == $permission_644 ]] \
&& grep -q "$msg_install" $log && [ -d $DataPath ] && [ -d $LogPath ] \
&& grep -q "Restart=on-failure" "$RE_master_systemd" && grep -q "Restart=on-failure" "$RE_ssl_systemd" \
