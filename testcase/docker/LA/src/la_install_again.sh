...
#date            :20190329                                                                                       #
#version         :1.0                                                                                            #
#usage           :./la_install_again.sh                                                                          #
#actual results  :Testcase la_install_again.sh passed!                                                           #
#expected results:Testcase la_install_again.sh passed!                                                           #
##################################################################################################################
...
log2="LA_installed.log"                                                                                          #installation log if installing License Agent again
...
msg_again="License Agent has already been installed on this machine."                                            #message for a sign of License Agent already installed
msg_installed="If you believe that License Agent has not been installed, \
please uninstall the rpm package netbrainlicense."                                                               #message for a sign of License Agent already installed
...
timeout 600 ./install.sh <$input >$log2 2>&1                                                                     #execute License Agent installation script again and save the log
if [ $? -eq 124 ];then                                                                                           #determine if the testcase runs timed out
    echo "Testcase $BASH_SOURCE timeout!"
   ...
   if grep -q "$msg_again" $log2 && grep -q "$msg_installed" $log2;then                                          #determine if License Agent installation is successfully completed
